...
const demoAccountId = 'CLINIC-0001';
...
const demoUsers = [
  ...
  { username: 'community', role: 'Community', email: 'community@demo.clinic', password: 'Password123!' },
];
...
async function seed() {
  try {
    ...
    const tenantInsert = await pool.query(
      `INSERT INTO tenants (id, slug, name_en, timezone, locale, active, created_at, updated_at, tenant_id, account_id, clinic_name, moh_license_number)
       VALUES ($1, $2, $3, 'Asia/Riyadh', 'ar-SA', true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, $4, $5, $6, $7)
       ON CONFLICT (slug) DO UPDATE SET account_id = EXCLUDED.account_id, clinic_name = EXCLUDED.clinic_name, moh_license_number = EXCLUDED.moh_license_number
       RETURNING tenant_id`,
      [
        'demo-clinic',
        'demo-clinic',
    ...
    const tenantId = tenantInsert.rows[0]?.tenant_id || demoTenantId;
    ...
    for (const user of demoUsers) {
      ...
      await pool.query(
        ...
        [tenantId, user.username, hash, user.email, user.role]
      );
    }
    ...
    console.log('Community username:', 'community', 'Password: Password123!');
  } catch (error) {
